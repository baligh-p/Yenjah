...
try {
    ...
    if (isset($_FILES["photo"])) {
        $target_file = "assets/photoPost/" . time() . basename($_FILES["photo"]["name"]);
    ...
    $id = generateID(30);
    $stmt = $conn->prepare("INSERT INTO post(text,titre,idPost,idProfile,objectif,idTypeGeneral,idTypeSpecifique,imagePost) VALUES (?,?,?,?,?,?,?)");
    $stmt->execute(array(
        $description,
        $title,
        $id,
